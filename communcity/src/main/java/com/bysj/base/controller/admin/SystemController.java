...
public class SystemController {
	...
	public Result<Boolean> login(HttpServletRequest request, User user, String cpacha, String type) {
		...
		if ("2".equals(type)) {
			...
			//表示验证码正确，开始查询数据库，检验密码是否正确
			Student findByStudentNo = studentService.findByStudentNo(user.getUsername());
			//判断是否为空
			if (findByStudentNo == null) {
				return Result.error(CodeMsg.ADMIN_USERNAME_NO_EXIST);
			}
			//表示用户存在，进一步对比密码是否正确
			if (!findByStudentNo.getPassword().equals(user.getPassword())) {
				return Result.error(CodeMsg.ADMIN_PASSWORD_ERROR);
			}
			user.setEmail(findByStudentNo.getEmail());
			user.setUsername(findByStudentNo.getStudentName());
			user.setRole(findByStudentNo.getRole());
			user.setMobile(findByStudentNo.getMobile());
			user.setSex(findByStudentNo.getStudentSex());
			user.setId(findByStudentNo.getId());
			user.setHeadPic(findByStudentNo.getHeadPic());
			//检查一切符合，可以登录，将用户信息存放至session
			request.getSession().setAttribute(SessionConstant.SESSION_USER_LOGIN_KEY, user);
			request.getSession().setAttribute("admin_login", null);
			request.getSession().setAttribute("admin_stu", findByStudentNo);
			//将登陆记录写入日志库
			operaterLogService.add("用户【" + user.getUsername() + "】于【" + StringUtil.getFormatterDate(new Date(), "yyyy-MM-dd HH:mm:ss") + "】登录系统！");
			log.info("用户成功登录，user = " + user);
		} else if ("1".equals(type)) {
			//表示验证码正确，开始查询数据库，检验密码是否正确
			Emp byNumber = empService.findByNumber(user.getUsername());
			//判断是否为空
			if (byNumber == null) {
				return Result.error(CodeMsg.ADMIN_USERNAME_NO_EXIST);
			}
			//表示用户存在，进一步对比密码是否正确
			if (!byNumber.getPassword().equals(user.getPassword())) {
				return Result.error(CodeMsg.ADMIN_PASSWORD_ERROR);
			}
			user.setEmail(byNumber.getEmail());
			user.setUsername(byNumber.getName());
			user.setPassword(byNumber.getPassword());
			user.setRole(byNumber.getRole());
			user.setMobile(byNumber.getMobile());
			user.setSex(byNumber.getSex());
			user.setHeadPic(byNumber.getHeadPic());
			user.setRole(byNumber.getRole());
			user.setId(byNumber.getId());
			//检查一切符合，可以登录，将用户信息存放至session
			request.getSession().setAttribute(SessionConstant.SESSION_USER_LOGIN_KEY, user);
			request.getSession().setAttribute("admin_login", null);
			//将登陆记录写入日志库
